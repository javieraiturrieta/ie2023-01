...
let tarea2Link = "https://wiki.ead.pucv.cl/Javiera_Iturrieta:_Encargo_02_IE_2023"; // Enlace para la tarea 2
let tarea1Hover = false; // Variable para detectar si el cursor está sobre "Tarea 1"
let tarea2Hover = false; // Variable para detectar si el cursor está sobre "Tarea 2"
...
function draw() {
  ...
  tarea1Hover = mouseX > tarea1TextX && mouseX < tarea1TextX + textWidth("Tarea 1") && mouseY > tarea1TextY - tarea1TextSize / 2 && mouseY < tarea1TextY + tarea1TextSize / 2;
  ...
  if (mouseIsPressed && tarea1Hover) {
    window.open(tarea1Link, "_blank");
  }

  // Verificar si el cursor está sobre el texto "Tarea 2"
  let tarea2TextX = 20;
  let tarea2TextY = height / 2 + 60; // Ligeramente debajo del texto anterior
  let tarea2TextSize = 14;
  tarea2Hover = mouseX > tarea2TextX && mouseX < tarea2TextX + textWidth("Tarea 2") && mouseY > tarea2TextY - tarea2TextSize / 2 && mouseY < tarea2TextY + tarea2TextSize / 2;

  // Cambiar el estilo del texto "Tarea 2" cuando el cursor está sobre él
  if (tarea2Hover) {
    fill(255); // Blanco
    cursor(HAND); // Cambiar el cursor a una mano
  } else {
  ...
  // Mostrar el texto "Tarea 2" y establecer enlace cuando se hace clic
  textSize(tarea2TextSize);
  textStyle(ITALIC);
  textAlign(LEFT, CENTER);
  text("Tarea 2", tarea2TextX, tarea2TextY);

  // Si se hace clic en "Tarea 2", abrir el enlace
  if (mouseIsPressed && tarea2Hover) {
    window.open(tarea2Link, "_blank");
  }
}
